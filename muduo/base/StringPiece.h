...
// 为什么要有这个StringPiece类?
// C++里面有string和char*，如果你用const string &s 做函数形参，可以同时兼容两种字符串。但当你传
// 入一个很长的char * 时，char*转string，创建一个临时string对象,同时开辟一个内存空间，拷贝字符串
// , 开销比较大。如果你的目的仅仅是读取字符串的值，用这个StringPiece的话，仅仅是4+一个指针的内存
// 开销，而且也保证了兼容性。所以这个类的目的是传入字符串的字面值，它内部的ptr_ 这块内存不归他所有。
// 所以不能做任何改动。归根结底，是处于性能的考虑，用以实现高效的字符串传递，这里既可以用const char*，
