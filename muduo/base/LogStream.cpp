...
#ifndef __STDC_FORMAT_MACROS // for PRId64
...
const char digits[] = "9876543210123456789"; // 余数可能为负
...
std::string formatSI(int64_t s)
{
	...
	char buf[64];
	if (s < 1000) snprintf(buf, sizeof buf, "%" PRId64, s);
	else if (s < 9995) 
...
{
	if (buffer_.avail() >= kMaxNumericSize){
		int len = snprintf(buffer_.current(), kMaxNumericSize, "%.12g", v); // 自动选择f,e格式
		buffer_.add(len);
	}
	return *this;
}
